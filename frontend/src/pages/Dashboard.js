    ...
    <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-zinc-800">
      ...
      <div className={`flex flex-col items-center justify-center transition-all duration-1000 ${data ? 'pt-8 pb-6' : 'pt-32 pb-12'}`}>
        <div className="text-center mb-10">
            <h1 className="text-4xl font-black tracking-tight text-white flex items-center gap-3 justify-center italic">
        ...
        <div className="w-full max-w-lg px-6">
            <div className="relative flex bg-zinc-900/50 border border-zinc-800 rounded-full p-1 focus-within:border-zinc-700 transition-colors">
                ...
                />
                <button onClick={handleSearch} className="bg-zinc-100 hover:bg-white text-black px-8 py-2 rounded-full font-bold transition-all text-xs">
                    {loading ? "..." : "ANALYZE"}
                </button>
            </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4">
              <div className="bg-zinc-950 border border-zinc-800 p-8 rounded-3xl flex flex-col items-start relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold text-white tracking-tight">{data.profile.name}</h2>
                <p className="text-zinc-500 text-xs mb-2">@{data.profile.username}</p>
                <p className="text-zinc-500 mt-2 text-sm leading-relaxed">{data.profile.bio || "No bio available."}</p>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    {data.profile.organization && (
                        <div className="flex items-center gap-2 px-3 py-1 bg-zinc-900/50 rounded-full border border-zinc-800/50">
                            ...
                            <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-tight">{data.profile.organization}</span>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 px-3 py-1 bg-zinc-900/50 rounded-full border border-zinc-800/50">
                        ...
                        <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-tight ml-1">Active {formatTimeAgo(data.profile.last_active)}</span>
                    </div>

                    {data.profile.is_pro && (
                        <div className="flex items-center gap-1.5 px-3 py-1 bg-purple-950/40 border border-purple-500/30 rounded-full">
                            <Star size={10} className="text-purple-400 fill-purple-400" />
                            <span className="text-[10px] text-purple-400 font-black uppercase tracking-tighter italic">@pro</span>
                        </div>
                    )}
                </div>
              </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-8 rounded-3xl h-[450px]">
